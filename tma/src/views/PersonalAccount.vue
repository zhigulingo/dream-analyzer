...
// Функция для определения, является ли устройство мобильным (если нужна для expand)
...
onMounted(async () => {
    ...
    if (tg) {
        ...
        // Это должно также повлиять на закрытие свайпом
        if (typeof tg.enableClosingConfirmation === 'function') {
            tg.enableClosingConfirmation();
            console.log("[PersonalAccount] Closing confirmation enabled.");
        } else {
            console.warn("[PersonalAccount] tg.enableClosingConfirmation is not a function. Feature might not be supported.");
        }

        // --- УПРАВЛЕНИЕ РАЗМЕРОМ ОКНА (ОПЦИОНАЛЬНО, если вы все же хотите expand на мобильных) ---
        // if (isMobileDevice()) {
        ...
        if (typeof tg.BackButton?.show === 'function' && typeof tg.BackButton?.onClick === 'function') {
            ...
            tg.BackButton.onClick(() => {
                console.log(`[PersonalAccount BackButton] Clicked. Modal: ${userStore.showSubscriptionModal}, Reward View: ${showRewardClaimView.value}, Closing Confirmation Enabled: ${tg.isClosingConfirmationEnabled}`);
                if (userStore.showSubscriptionModal) {
                    userStore.closeSubscriptionModal();
                } else if (showRewardClaimView.value === true) {
                    goBackToAccount();
                } else {
                    // Поскольку enableClosingConfirmation включено, Telegram сам обработает диалог подтверждения.
                    // Нам просто нужно вызвать close().
                    console.log("[PersonalAccount BackButton] Attempting to close TMA (confirmation should appear if enabled).");
