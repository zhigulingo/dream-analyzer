  <Teleport to="body">
    <div class="fixed inset-0 z-[10000] flex flex-col justify-end sm:justify-center items-center bg-black/80 backdrop-blur-sm overflow-y-auto" @click.self="closeModal">
      <!-- Touch area at top to close on mobile -->
      <div class="flex-1 w-full min-h-[15vh] sm:hidden" @click="closeModal"></div>
      
      <div class="w-full max-w-lg pointer-events-auto">
        <!-- Card Container -->
        <div class="bg-gradient-to-br from-[#8000FF] to-[#5500AA] text-white shadow-2xl w-full rounded-t-[32px] sm:rounded-[32px] overflow-hidden flex flex-col">
          ...
          <!-- Content area (scrollable if needed, but fixed in bot context usually) -->
          <div class="px-6 pb-8 flex flex-col gap-6">
             ...
             <!-- Bottom padding for mobile safe area / MainButton -->
             <div class="h-10 sm:h-0"></div>
          </div>
        </div>
        <!-- Extra bottom filling for the swipe area on some devices -->
        <div class="h-20 sm:hidden bg-[#5500AA]"></div>
      </div>
    </div>
  </Teleport>
