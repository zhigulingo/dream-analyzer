    ...
    deep_json: `Ты — эмпатичный аналитик снов. Проанализируй серию из ${REQUIRED_DREAMS} снов и верни СТРОГО JSON с полями:
{
  ...
  "symbolsIntro": string,      // Вводный текст о повторяющихся символах (2-3 предложения, объясняет значимость повторений)
  
  "recurringSymbols": [
    {
      "symbol": string,          // Название символа (1-2 слова, БЕЗ emoji)
      "frequency": number,       // Количество снов из ${REQUIRED_DREAMS}, в которых встречается (ТОЛЬКО символы с частотой >= 2)
  ...
  "dynamicsContext": [
    {
      ...
      "values": number[],        // Процентное соотношение (0-100) для каждого из ${REQUIRED_DREAMS} снов, как в контентном анализе HVdC
...
- dynamicsContext: ОБЯЗАТЕЛЬНО ВСЕ 4 категории ("Персонажи", "Эмоции", "Действия", "Сцены") с РЕАЛЬНЫМИ ПРОЦЕНТАМИ (0-100) для КАЖДОГО из ${REQUIRED_DREAMS} снов, как в контентном анализе HVdC. В сумме 4 категории должны давать примерно 100% для каждого сна.
