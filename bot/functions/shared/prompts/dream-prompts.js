    ...
    deep_json: `Ты — эмпатичный аналитик снов. Проанализируй серию из ${REQUIRED_DREAMS} снов и верни СТРОГО JSON с полями:
{
  ...
  "analysis": string, // Общий контекст серии - основной текст анализа
  "overallContext": {
    "narrative": string,        // Общая история серии (2-3 предложения)
    "psychologicalFunction": string,  // Какую психологическую задачу решают эти сны
    "emotionalJourney": string  // Эмоциональный путь через серию снов
  },
  "recurringSymbols": [
    {
      "symbol": string,     // Название символа (1-2 слова, БЕЗ emoji)
      "occurrences": number,  // Количество появлений
      "interpretation": string  // Краткая интерпретация (1-2 предложения)
    }
  ],
  "dynamics": [
    {
      "metric": string,     // Название метрики (например: "Тревожность", "Контроль", "Ясность", "Активность")
      "values": number[],   // Значения от 0 до 10 для каждого из ${REQUIRED_DREAMS} снов
      "interpretation": string  // Что показывает эта динамика (2-3 предложения)
    }
  ],
  "conclusions": string[],  // 3 лаконичных НО содержательных инсайта (каждый - 15-25 слов, НЕ поверхностно)
  "recommendations": [
    {
      "title": string,      // Краткое название рекомендации (3-5 слов)
      "description": string,  // Подробное объяснение (3-4 предложения) - ЧТО конкретно делать
      "rationale": string   // РАЗВЕРНУТОЕ обоснование (2-3 предложения) - ЗАЧЕМ это делать на основе анализа снов
    }
  ]
}
...
- Теги и символы БЕЗ emoji, только текст (например: "Вода", "Преследование", "Дом").
- dynamics: 2-4 значимые метрики с РЕАЛЬНЫМИ числами от 0 до 10 для КАЖДОГО из ${REQUIRED_DREAMS} снов.
- conclusions: массив из 3-5 содержательных выводов (каждый - развернутый абзац).
- recommendations: массив из 3-4 практических рекомендаций с четким обоснованием.`
,
